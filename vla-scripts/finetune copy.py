...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            # Optimizer Step (only trigger actual gradient update after grad_accumulation_steps micro-batches)
            if (batch_idx + 1) % cfg.grad_accumulation_steps == 0:
                optimizer.step()
                scheduler.step()

                # Get actual gradient step index (starting from 1)
                gradient_step_idx = (batch_idx + 1) // cfg.grad_accumulation_steps

                # Compute smoothened train metrics for this gradient step
